...
var paths = {
  js: [
    ...
    '!packages/core/**/*.js',
    '!packages/core/public/assets/lib/**/*.js'
  ],
  html: [
  ...
  css: [
    ...
    '!packages/core/**/public/**/css/*.css'
  ],
  less: ['**/public/**/css/*.less'],
  sass: ['**/public/**/css/*.scss'],
  coffee: [
    'packages/**/public/**/*.coffee',
    '*.coffee'
  ],
...
gulp.task('jshint', function () {
  return gulp.src(paths.js).pipe(plugins.jshint()).pipe(plugins.jshint.reporter('jshint-stylish')).pipe(plugins.jshint.reporter('fail')).pipe(count('jshint',
    'files lint free'));
});

gulp.task('csslint', function () {
  return gulp.src(paths.css).pipe(plugins.csslint('.csslintrc')).pipe(plugins.csslint.reporter()).pipe(count('csslint',
    'files lint free'));
});

gulp.task('less', function () {
  return gulp.src(paths.less).pipe(plugins.less()).pipe(gulp.dest(function (vinylFile) {
...
gulp.task('coffee', function () {
  gulp.src(paths.coffee).pipe(coffee({bare: true}).on('error',
    gutil.log)).pipe(gulp.dest('./packages'))
});
