...
function install(module, options) {
  ...
  loadPackageJson('./node_modules/meanio/package.json', function (err, data) {
    ...
    getPackage(packageName, function (data) {
      ...
      var cloneUrl = options.git ? data.repo : 'https://git.mean.io/' +
                                               data.repo.split(':')[1];

      shell.rm('-rf', destination + data.name);

      console.log('git clone --branch ' + tag + ' ' + cloneUrl + ' ' +
                  destination + data.name);

      var silentState = shell.config.silent; // save old silent state
      shell.config.silent = true;

      shell.exec('git clone --branch ' + tag + ' --depth 1 ' + cloneUrl + ' ' +
                 destination + data.name, function (code) {
  ...
  npm.load({
    ...
    fs.readdir(packages, function (err, files) {
      ...
      files.forEach(function (file) {
        ...
        loadPackageJson(path.join(pkgPath, 'package.json'),
          function (err, data) {
